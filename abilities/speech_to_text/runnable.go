...
func (r *Runnable) samplesFunc(s Samples) func() {
	return func() {
		...
		vss := sd.Add(s.Samples, s.SampleRate, s.MaxSilenceAudioLevel)
...
func (r *Runnable) buildReferences(rw http.ResponseWriter, req *http.Request, p httprouter.Params) {
	...
	r.ms.Lock()
	var ks int64Slice
	ss := make(map[int64][]Speech)
	for _, s := range r.ss {
		if _, ok := ss[s.CreatedAt.UnixNano()]; !ok {
			ks = append(ks, s.CreatedAt.UnixNano())
		}
		ss[s.CreatedAt.UnixNano()] = append(ss[s.CreatedAt.UnixNano()], *s)
	}
	r.ms.Unlock()
	...
	for _, k := range ks {
		...
		for _, s := range ss[k] {
			rf.Speeches = append(rf.Speeches, s)
		}
	}
